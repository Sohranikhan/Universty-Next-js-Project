...
import { getDownloads } from "../../../../utils/getData"; // Update the function to fetch downloads
import { FaBug } from "react-icons/fa";
import DataTable from "./Components/DataTable";

const DownloadsPage = async () => {
    const data = await getDownloads(); // Fetch downloads data
            ...
            <div className="w-full flex flex-col h-full items-center justify-center gap-y-2 sm:p-6 p-3 text-foreground">
                <div className="w-full flex items-center justify-between gap-2">
                    <p className="flex items-center gap-3 text-lg font-bold">
                        <BoxIcon /> No Download Found
                    </p>
                    <BtnLink text={'Create New'} href={'/dashboard/downloads/new'} />
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="w-full h-auto sm:p-6 p-3">
                <div className="flex items-center justify-between">
                    <Headings title={'Downloads'} des={'List of Downloads'} />
                    <BtnLink text={'Create New'} href={'/dashboard/downloads/new'} />
                </div>
                <DataTable data={data.data} /> {/* Pass data to DataTable */}
            </div>
        );
    } else {
        return (
