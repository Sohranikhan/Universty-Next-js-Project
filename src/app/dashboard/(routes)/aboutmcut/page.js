...
import { getAboutMcut } from "../../../../utils/getData"
import { BoxIcon } from "lucide-react"
import { FaBug } from "react-icons/fa"
import DataTable from "./Components/DataTable"


const aboutMcut = async () => {

    const data = await getAboutMcut()
        ...
        return (
            <div className="w-full flex flex-col h-full items-center justify-center gap-y-2 sm:p-6 p-3 text-foreground">
                <div className="w-full flex items-center justify-between gap-2">
                    <p className="flex items-center gap-3 text-lg font-bold"><BoxIcon /> No About Page Found </p>
                    <BtnLink text={'Create New'} href={'/dashboard/aboutmcut/new'} />
                </div>
            </div>
        )
    }
        ...
        return (
            <div className="w-full h-auto sm:p-6 p-3">
                <div className="flex items-center justify-between">
                    <Headings title={'About'} des={'About Pages of MCUT'} />
                    <BtnLink text={'Create New'} href={'/dashboard/aboutmcut/new'} />
                </div>
